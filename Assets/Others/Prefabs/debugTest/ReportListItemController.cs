...
public class ReportListItemController : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        if (reportText.text.Length > 0) imagePathBg.SetActive(true);
        ...
        if (ImageUrl != "" && !imagedownloaded && check_if_isvisible_bycamera(gameObject))
        {
            if (!isdownloading) StartCoroutine(downloadImage(ImageUrl, sprite =>
            {
                try
                {
                    ...
                    gameObject.GetComponent<ReportListItemController>().imageJsonString = Json.ToString();
                    gameObject.GetComponent<ReportListItemController>().idText.text = Json.GetString("ImageName");
                    gameObject.GetComponent<ReportListItemController>().reportText.text = "تعداد گزارش : " + Json.GetNumber("ReportsCount");

                    ReportManager.getInstance.downloadedImagesCount++;
    ...
    public void updateReport()
    {
        ...
        if (imagedownloaded) ReportManager.getInstance.updateReport(GetComponent<Image>().sprite, imageJsonString);
    }
